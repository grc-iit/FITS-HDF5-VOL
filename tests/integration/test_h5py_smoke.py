...
Verifies the path: h5py opens a FITS file through fits-hdf5-vol via env-var
...
if "FITS_FIXTURES_DIR" not in os.environ:
    print("FAIL: FITS_FIXTURES_DIR env var not set", file=sys.stderr)
    sys.exit(1)

path = os.path.join(os.environ["FITS_FIXTURES_DIR"], "image_2d.fits")
...
print(f"OK: h5py {h5py.__version__} (HDF5 {h5py.version.hdf5_version}) "
      f"opens FITS via fits-hdf5-vol and reads attributes by path")
sys.exit(0)
